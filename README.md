...
- 战绩记录与排行榜：记录每局桌游的参与者、胜负结果、时长，生成个人胜率排行榜和常用桌游统计，玩家可查看自己的桌游生涯数据。
  - 仅已结束且至少 2 名不同玩家的对局参与统计（`POST /api/matches`，需携带 `X-Request-Id` 幂等键）
  - 修正对局（`PUT /api/matches/:id`，基于 `expectedRevision` 乐观锁）时旧版本胜负先完整冲销再计入新结果
  - 并发重复提交不重复计分；写流水任一步失败都会按对局源记录自愈，重启时再次对账，排行榜（`GET /api/leaderboard`）始终与源记录一致
